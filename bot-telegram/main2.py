...
def agent():
    agents = random.choice(open("useragent2.txt", 'r').readlines()).replace("\n", "")
    return agents
...
if (os.path.exists("monitor2.json")):
    previous_monitor = open("monitor2.json")
...
def iterateDomains(domains, sleep_time=5, timeout=180):
    ...
    json_file = open("monitor2.json", "w")
    json_file.write(json_f)
    json_file.close()
while (True):
    try:
        domains = open("domain2.txt", 'r').readlines()
        telegram_bot_sendtext_untuk_agungsurya("script2 mulai berjalan") #print ("script mulai berjalan")  #telegram_bot_sendtext_untuk_agungsurya("script mulai berjalan") #telegram_bot_sendtext("script mulai berjalan")
        start = time.time()
        ...
        print(f"program2 telah selesai dengan waktu {time_required} sekon")
    except Exception as e: 
